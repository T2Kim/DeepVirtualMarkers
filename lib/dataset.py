...
class DataBag:
  ...
  def setcapacity(self, num):
    self.cached_data = [None]*num

  def getcapacity(self):
    return len(self.cached_data)

  def getdata(self, idx):
    return self.cached_data[idx][0], self.cached_data[idx][1]
...
class VoxelizationRenderedDataset(VoxelizationDatasetBase):
  """This dataset loads RGB point clouds and their labels as a list of points
  and voxelizes the pointcloud with sufficient data augmentation.
  """
  # Voxelization arguments
  ...
  def __getitem__(self, index):
    # coords, feats, labels, center = self.load_ply(index)
    coords, labels = self.dataBag.getdata(index)

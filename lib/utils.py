...
def save_soft_predictions_color(coords, upsampled_pred, transformation, dataset, config, iteration, save_pred_dir):
  # Note! : batch size 1
  coords = coords.astype(np.float32)
  coords *= 0.01

  cv2gl = np.array([[1.,0,0],[0,-1,0],[0,0,-1]])
  coords = np.dot(coords, cv2gl)

  upsampled_pred = upsampled_pred.astype(np.float32)
  upsampled_pred_l = upsampled_pred.sum(axis=1)
  upsampled_pred = upsampled_pred / upsampled_pred_l[:, np.newaxis]
  v_color = np.zeros((len(coords), 3), dtype=np.float32)

  for i in range(upsampled_pred.shape[1]):
      v_color += np.dot((upsampled_pred[:, i])[:, np.newaxis],
                      np.array(COLOR_MAP[i], dtype=np.float32).reshape(1, 3))
  # Determine full path of the destination.
  filename = 'pred_%06d.ply' % (iteration)
  pcd_color = np.hstack((coords[:, :3],v_color))
  save_point_cloud(pcd_color,os.path.join(save_pred_dir, filename), verbose=False)
  return pcd_color


def save_soft_pred_gt_color(coords, upsampled_pred, ground_truth, transformation, dataset, config, iteration, save_pred_dir):
  # Note! : batch size 1
  coords = coords.astype(np.float32)
  coords *= 0.01
  ...
  # ground_truth_l = ground_truth.sum(axis=1)
  # ground_truth = ground_truth / ground_truth_l[:, np.newaxis]
  
  # get max
  # ar_max = np.argmax(upsampled_pred, axis=1)
  # arr = list(range(upsampled_pred.shape[0]))
  # upsampled_pred[:,:] = 0
  # upsampled_pred[arr, ar_max] = 1

  v_color = np.zeros((len(coords), 3), dtype=np.float32)
  gt_color = np.zeros((len(coords), 3), dtype=np.float32)

  for i in range(upsampled_pred.shape[1]):
      v_color += np.dot((upsampled_pred[:, i])[:, np.newaxis],
                      np.array(COLOR_MAP[i], dtype=np.float32).reshape(1, 3))
  for i in range(ground_truth.shape[1]):
      gt_color += np.dot((ground_truth[:, i])[:, np.newaxis],
                      np.array(COLOR_MAP[i], dtype=np.float32).reshape(1, 3))
  # Determine full path of the destination.
  filename = 'pred_%06d.ply' % (iteration)
  filename2 = 'gt_%06d.ply' % (iteration)
  pcd_color = np.hstack((coords[:, :3],v_color))
  pcd_color2 = np.hstack((coords[:, :3],gt_color))
  save_point_cloud(pcd_color,os.path.join(save_pred_dir, filename), verbose=False)
  save_point_cloud(pcd_color2,os.path.join(save_pred_dir, filename2), verbose=False)
  return pcd_color, pcd_color2
...
def save_predictions(coords, upsampled_pred, transformation, dataset, config, iteration,
  ...
  for i in range(iter_size):
    ...
    filename = 'pred_%06d_%03d.npy' % (iteration, i)
    if dataset.IS_TEMPORAL:
      filename = 'pred_%06d_%03d_%03d.npy' % (iteration, i, j)
